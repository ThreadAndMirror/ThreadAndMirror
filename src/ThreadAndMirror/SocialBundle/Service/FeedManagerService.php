...
use ThreadAndMirror\SocialBundle\Entity\Post;
...
class FeedManagerService extends ContainerAware
{
     ...
     * @return array
     */
    public function getPostByShortcode($shortcode)
    {
        $token  = $this->container->getParameterBag()->get('threadandmirror.social.instagram.token');

        $url = sprintf('https://api.instagram.com/v1/media/shortcode/%s?access_token=%s', $shortcode, $token);

        $json = $this->apiRequest($url);

        return json_decode(json_encode($json->data), true);
    }
     ...
     */
    private function apiRequest($url, $options = array())
    {
        $ch = curl_init();
