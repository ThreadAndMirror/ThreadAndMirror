...
namespace ThreadAndMirror\MoodBoardBundle\Service;

use Doctrine\ORM\EntityManager,
	Symfony\Bundle\TwigBundle\TwigEngine,
	Symfony\Component\Form\FormFactoryInterface,
	Symfony\Component\DependencyInjection\ContainerAware,
	ThreadAndMirror\MoodBoardBundle\Entity\Post;

/**
 *	Functionality for parsing, updating and moderating MoodBoard feeds
 */
class FeedManagerService extends ContainerAware
{
	...
	public function updateFeeds($type=null)
	{
		...
		if ($type && in_array($type, $this->container->getParameterBag()->get('threadandmirror.MoodBoard.feed_types'))) {
			...
			foreach ($this->container->getParameterBag()->get('threadandmirror.MoodBoard.feed_types') as $type) {
				$method = 'update'.ucfirst($type).'Feeds';
				$errors += $this->$method();
			}
	...
	protected function updateTwitterFeeds()
	{
		...
		$token  = $this->container->getParameterBag()->get('threadandmirror.MoodBoard.twitter.token');
		$errors = 0;

		// get the active feed owners
		$owners   = $this->em->getRepository('ThreadAndMirrorMoodBoardBundle:Feed')->findActiveFeeds('twitter');
		$existing = $this->em->getRepository('ThreadAndMirrorMoodBoardBundle:Post')->getExistingPostIds();
	...
	{
		// Get our config controlled API params
		$token  = $this->container->getParameterBag()->get('threadandmirror.MoodBoard.tumblr.token');
		$errors = 0;

		// Get the active feed owners
		$owners   = $this->em->getRepository('ThreadAndMirrorMoodBoardBundle:Feed')->findActiveFeeds('tumblr');
		$existing = $this->em->getRepository('ThreadAndMirrorMoodBoardBundle:Post')->getExistingPostIds();
	...
	{
		// get our config controlled API params
		$client = $this->container->getParameterBag()->get('threadandmirror.MoodBoard.instagram.client');
		$token  = $this->container->getParameterBag()->get('threadandmirror.MoodBoard.instagram.token');
		$count  = $this->container->getParameterBag()->get('threadandmirror.MoodBoard.instagram.count');
		$errors = 0;

		// get the active feed owners and existing post sids
		$owners   = $this->em->getRepository('ThreadAndMirrorMoodBoardBundle:Feed')->findActiveFeeds('instagram');
		$existing = $this->em->getRepository('ThreadAndMirrorMoodBoardBundle:Post')->getExistingPostIds();
