...
class RestController extends BaseRestController
{
	...
	public function favouriteFeedAction($id)
	{
		...
		if (!$this->get('security.context')->isGranted('ROLE_USER')) {
			return $this->error('You need to be logged in before you can add to your MoodBoard circle.', true)->sendResponse();
		}
		...
		$feed 	 = $em->getRepository('ThreadAndMirrorMoodBoardBundle:Feed')->find($id);

		// Check that a feed with that id exists
		if (!$feed) {
			return $this->error('We couldn\'t find the requested MoodBoard circle!', true)->sendResponse();
		}

		// Favourites can be null if none exist already
		$favourites = $profile->getMoodBoardFeeds() ? $profile->getMoodBoardFeeds() : array();
		...
		if (!in_array($id, $favourites)) {
			...
			$favourites[] = $id;
			$profile->setMoodBoardFeeds($favourites);
			$em->persist($profile);
			$em->flush();

			return $this->success($feed->getName().' has been added to your MoodBoard circle.', true)->sendResponse();

		} else {

			// Notify that it's already a favourite
			return $this->error($feed->getName().' is already in your MoodBoard circle!', true)->sendResponse();
		}	
	}
	...
	public function getMorePostsAction($source='all', $offset, $chunk=50)
	{
		...
		// Get the posts
		if ($source == 'all') {
			$posts = $em->getRepository('ThreadAndMirrorMoodBoardBundle:Post')->findBy(array('deleted' => false), array('created' => 'DESC'), $chunk, $offset);
		} else {
			$posts = $em->getRepository('ThreadAndMirrorMoodBoardBundle:Post')->findBy(array('source' => $source, 'deleted' => false), array('created' => 'DESC'), $chunk, $offset);
		}
		...
		foreach ($posts as &$post) {
			$html .= $this->renderView('ThreadAndMirrorMoodBoardBundle:Rest:post.html.twig', array(
				'post' 		=> $post,
			));
		}
	...
	public function getMoreCategoryPostsAction($category='magazine', $offset, $chunk=50)
	{
		// Get the posts
		$em    = $this->getDoctrine()->getManager();
		$posts = $em->getRepository('ThreadAndMirrorMoodBoardBundle:Post')->findByOwnerCategory($category, $offset, $chunk);
	...
	public function getMoreFeedPostsAction($id, $offset, $chunk=50)
	{
		// Get the posts
		$em    = $this->getDoctrine()->getManager();
		$posts = $em->getRepository('ThreadAndMirrorMoodBoardBundle:Post')->findBy(array('feed' => $id, 'deleted' => false), array('created' => 'DESC'), $offset, $chunk);
	...
	public function getMoreFavouritePostsAction($offset, $chunk=50)
	{
		...
		$em      = $this->getDoctrine()->getManager();
		$profile = $em->getRepository('ThreadAndMirrorProductsBundle:Profile')->findOneByUser($this->getUser()->getId());
		$posts   = $em->getRepository('ThreadAndMirrorMoodBoardBundle:Post')->findFromFeedList($profile->getMoodBoardFeeds(), $offset, $chunk);
